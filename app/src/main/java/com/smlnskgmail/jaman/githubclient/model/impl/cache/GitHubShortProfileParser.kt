...
class GitHubShortProfileParser(
    ...
    fun fromJsonString(): GitHubShortProfile {
        val gitHubShortProfileJson = JsonParser().parse(content).asJsonObject
        val id = gitHubShortProfileJson.get(idKey).asInt
        val login = gitHubShortProfileJson.get(loginKey).asString
        val profileType = gitHubShortProfileJson.get(typeKey).asString
        val photoUrl = if (gitHubShortProfileJson.has(photoUrlKey)) {
            gitHubShortProfileJson.get(photoUrlKey).asString
        } else {
            null
        }
